...
import { Bien, Contrat, Locataire, Paiement, Proprietaire } from '@habitia/types';
...
interface ReceiptData {
  ...
  contrat: Contrat;
  paiement: Paiement;
  periode: string; // e.g. "Juin 2026"
}

const addLogoToPdf = (doc: any, logoDataUrl: string | undefined | null, x: number, y: number, width: number): boolean => {
...
export const generateQuittancePDF = (res: Response, data: ReceiptData) => {
...
export const generateAccountingReportPDF = (res: Response, data: any) => {
  const doc = new PDFDocument({ size: 'A4', margin: 50 });
  doc.pipe(res);
  ...
  transactions.slice(0, 20).forEach((t: any) => { // limit to first 20 for 1-page report
