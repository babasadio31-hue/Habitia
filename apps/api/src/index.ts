...
function mapBienStatutToDb(statut: string): any {
  if (statut === 'loué') return 'loue';
  return statut;
}

function mapBienStatutToFe(statut: string): string {
  if (statut === 'loue') return 'loué';
  return statut;
}

function mapPaiementStatutToDb(statut: string): any {
  if (statut === 'payé') return 'paye';
  return statut;
}

function mapPaiementStatutToFe(statut: string): string {
  if (statut === 'paye') return 'payé';
  return statut;
}

function mapChantierStatutToDb(statut: string): any {
  if (statut === 'planifié') return 'planifie';
  if (statut === 'terminé') return 'termine';
  return statut;
}

function mapChantierStatutToFe(statut: string): string {
...
app.get('/api/dashboard/stats', authMiddleware, async (req, res) => {
  try {
    const totalBiens = await db.bien.count();
    const activeContractsCount = await db.contrat.count({ where: { actif: true } });
    const biensLoues = activeContractsCount;
    
    // Active contracts
    const activeContracts = await db.contrat.findMany({ where: { actif: true } });
    ...
    const paymentsJune = await db.paiement.findMany({
      where: {
        datePaiement: {
          gte: startJune,
          lte: endJune
    ...
    const chantiersEnCours = await db.chantier.count({ where: { statut: 'en_cours' } });
    ...
    const countEnTravaux = await db.bien.count({ where: { statut: 'en_travaux' } });
    const countEnVente = await db.bien.count({ where: { statut: 'en_vente' } });
    ...
    const payments = await db.paiement.findMany({
...
app.get('/api/proprietaires', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const where = searchStr ? {
      OR: [
        { nom: { contains: searchStr, mode: 'insensitive' as const } },
        { prenom: { contains: searchStr, mode: 'insensitive' as const } },
    ...
    const total = await db.proprietaire.count({ where });
    const list = await db.proprietaire.findMany({
      where,
      skip: offset,
      take: limitNum,
      include: {
...
app.post('/api/proprietaires', authMiddleware, requireRole(['admin', 'agent']), async (req, res) => {
  ...
  try {
    const newProp = await db.proprietaire.create({
      data: {
        nom,
        prenom,
        email,
...
app.get('/api/proprietaires/:id/compte-rendu', authMiddleware, async (req, res) => {
  const { id } = req.params;

  try {
    const owner = await db.proprietaire.findUnique({
      where: { id }
    });
    ...
    const ownerBiens = await db.bien.findMany({
      where: { proprietaireId: id }
    });
    const bienIds = ownerBiens.map(b => b.id);
    const activeContracts = await db.contrat.findMany({
...
app.put('/api/proprietaires/:id', authMiddleware, requireRole(['admin', 'agent']), async (req, res) => {
  const { id } = req.params;

  try {
    const updated = await db.proprietaire.update({
      where: { id },
      data: req.body
    });
...
app.delete('/api/proprietaires/:id', authMiddleware, requireRole(['admin']), async (req, res) => {
  const { id } = req.params;

  try {
    await db.proprietaire.delete({ where: { id } });
...
app.get('/api/proprietaires/:id/retraits', authMiddleware, async (req, res) => {
  const { id } = req.params;
  try {
    const retraits = await db.retrait.findMany({
      where: { proprietaireId: id },
...
app.post('/api/proprietaires/:id/retraits', authMiddleware, requireRole(['admin', 'agent', 'comptable']), async (req, res) => {
  const { id } = req.params;
  ...
  try {
    const newRetrait = await db.retrait.create({
      data: {
...
app.delete('/api/proprietaires/:ownerId/retraits/all', authMiddleware, requireRole(['admin', 'agent', 'comptable']), async (req, res) => {
  const { ownerId } = req.params;
  try {
    await db.retrait.deleteMany({ where: { proprietaireId: ownerId } });
...
app.delete('/api/proprietaires/:ownerId/retraits/:id', authMiddleware, requireRole(['admin', 'agent', 'comptable']), async (req, res) => {
  const { id } = req.params;
  try {
    await db.retrait.delete({ where: { id } });
...
app.get('/api/locataires', authMiddleware, async (req, res) => {
  const { search, limit = '10', page = '1' } = req.query;
  ...
  try {
    ...
    const total = await db.locataire.count({ where });
    const list = await db.locataire.findMany({
      where,
      skip: offset,
      take: limitNum,
      include: {
...
app.post('/api/locataires', authMiddleware, requireRole(['admin', 'agent']), async (req, res) => {
  ...
  try {
    const newLoc = await db.locataire.create({
      data: {
        nom,
        prenom,
        email,
    ...
    if (bienId) {
      const bien = await db.bien.findUnique({ where: { id: bienId } });
      if (bien) {
...
app.put('/api/locataires/:id', authMiddleware, requireRole(['admin', 'agent']), async (req, res) => {
  const { id } = req.params;
  const { nom, prenom, email, telephone, employeur, garant, statut, bienId, loyer, caution, dateDebut } = req.body;

  try {
    const updatedLoc = await db.locataire.update({
      where: { id },
      data: {
        nom,
        prenom,
    ...
    const activeContract = await db.contrat.findFirst({
    ...
    if (bienId) {
      if (activeContract) {
        if (activeContract.bienId !== bienId) {
          ...
          const newBien = await db.bien.findUnique({ where: { id: bienId } });
          ...
          if (finalLoyer !== undefined) dataToUpdate.montantLoyer = finalLoyer;
          if (finalCaution !== undefined) dataToUpdate.depot = finalCaution;
...
app.delete('/api/locataires/:id', authMiddleware, requireRole(['admin']), async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    await db.locataire.delete({ where: { id } });
...
app.get('/api/contrats', authMiddleware, async (req, res) => {
  try {
    const list = await db.contrat.findMany({
      include: {
        locataire: true,
        bien: true
      }
...
app.get('/api/biens', authMiddleware, async (req, res) => {
  const { type, statut, ville, proprietaireId } = req.query;

  try {
    const where: any = {};
    if (type) where.type = type as any;
    if (statut) where.statut = mapBienStatutToDb(statut as string);
    if (ville) where.ville = { equals: String(ville), mode: 'insensitive' };
    if (proprietaireId) where.proprietaireId = String(proprietaireId);

    const list = await db.bien.findMany({
      where,
      include: { 
        proprietaire: true,
...
app.post('/api/biens', authMiddleware, requireRole(['admin', 'agent']), async (req, res) => {
  ...
  try {
    const newBien = await db.bien.create({
      data: {
        adresse,
        ville,
        type: type as any,
...
app.put('/api/biens/:id', authMiddleware, requireRole(['admin', 'agent']), async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    const updated = await db.bien.update({
      where: { id },
      data: {
        ...dataToUpdate,
...
app.delete('/api/biens/:id', authMiddleware, requireRole(['admin']), async (req, res) => {
  const { id } = req.params;

  try {
    await db.bien.delete({ where: { id } });
...
app.get('/api/chantiers', authMiddleware, async (req, res) => {
  try {
    const list = await db.chantier.findMany({
      include: { bien: true }
    });
...
app.post('/api/chantiers', authMiddleware, requireRole(['admin', 'agent']), async (req, res) => {
  ...
  try {
    const newChantier = await db.chantier.create({
      data: {
        bienId,
        titre,
...
app.put('/api/chantiers/:id', authMiddleware, requireRole(['admin', 'agent']), async (req, res) => {
  const { id } = req.params;
...
app.delete('/api/chantiers/:id', authMiddleware, requireRole(['admin']), async (req, res) => {
  const { id } = req.params;
...
app.get('/api/compta/transactions', authMiddleware, async (req, res) => {
  try {
    ...
    const expenses = await db.depense.findMany();
    ...
    const allTransactions = [...incomes, ...expTrans].sort(
...
app.post('/api/compta/transactions', authMiddleware, requireRole(['admin', 'comptable']), async (req, res) => {
  ...
  try {
    const newPaiement = await db.paiement.create({
      data: {
        contratId,
...
app.get('/api/compta/rent-roll', authMiddleware, async (req, res) => {
  try {
    const activeContracts = await db.contrat.findMany({
      where: { actif: true },
      include: {
...
// Balance par bien
app.get('/api/compta/balance', authMiddleware, async (req, res) => {
  try {
    const biens = await db.bien.findMany({
      include: {
        contrats: {
          include: {
            paiements: true
...
app.get('/api/compta/charts', authMiddleware, async (req, res) => {
  try {
    const rentPayments = await db.paiement.findMany({
      where: { type: 'loyer', statut: 'paye' },
      include: { contrat: { include: { bien: { include: { proprietaire: true } } } } }
    });
    const sales = await db.vente.findMany();
    const manualRevs = await db.revenuManuel.findMany();
    const expenses = await db.depense.findMany();

    const months = ['Jul 25', 'Aug 25', 'Sep 25', 'Oct 25', 'Nov 25', 'Dec 25', 'Jan 26', 'Feb 26', 'Mar 26', 'Apr 26', 'May 26', 'Jun 26'];
    const data = months.map((m, idx) => {
      let year = 2025;
      if (m.endsWith('26')) year = 2026;
...
app.get('/api/compta/dashboard', authMiddleware, async (req, res) => {
  try {
    const rentPayments = await db.paiement.findMany({
      where: { type: 'loyer', statut: 'paye' },
      include: { contrat: { include: { bien: { include: { proprietaire: true } } } } }
...
app.get('/api/compta/revenus', authMiddleware, async (req, res) => {
  try {
    const payments = await db.paiement.findMany({
      where: { type: 'loyer', statut: 'paye' },
    ...
    const allRevenues = [...rentCommissions, ...saleCommissions, ...manualList].sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
    );
...
app.post('/api/compta/revenus', authMiddleware, requireRole(['admin', 'comptable']), async (req, res) => {
  ...
  try {
    const newRev = await db.revenuManuel.create({
      data: {
        type,
        description,
        montant: parseFloat(montant),
...
app.delete('/api/compta/revenus/:id', authMiddleware, requireRole(['admin', 'comptable']), async (req, res) => {
  const { id } = req.params;
  try {
    const manual = await db.revenuManuel.findUnique({ where: { id } });
    if (manual) {
      await db.revenuManuel.delete({ where: { id } });
      return res.json({ message: 'Revenu manuel supprimé avec succès' });
    }

    const payment = await db.paiement.findUnique({ where: { id } });
    if (payment) {
      await db.paiement.delete({ where: { id } });
      return res.json({ message: 'Paiement supprimé avec succès' });
    }

    const sale = await db.vente.findUnique({ where: { id } });
    if (sale) {
...
app.get('/api/compta/depenses', authMiddleware, async (req, res) => {
  try {
    const list = await db.depense.findMany({
      orderBy: { date: 'desc' }
    });
...
app.post('/api/compta/depenses', authMiddleware, requireRole(['admin', 'comptable']), async (req: AuthenticatedRequest, res) => {
  const { categorie, description, montant, date, enregistrePar } = req.body;
  if (!categorie || !description || !montant || !date) {
    return res.status(400).json({ error: 'Champs obligatoires manquants' });
  }
  try {
    const newDep = await db.depense.create({
      data: {
        categorie,
        description,
        montant: parseFloat(montant),
...
app.delete('/api/compta/depenses/:id', authMiddleware, requireRole(['admin', 'comptable']), async (req, res) => {
  const { id } = req.params;
  try {
...
app.get('/api/compta/commissions', authMiddleware, async (req, res) => {
  try {
    const rentPayments = await db.paiement.findMany({
      where: { type: 'loyer' },
      include: { contrat: { include: { locataire: true, bien: { include: { proprietaire: true } } } } }
    ...
    const agents = await db.personnel.findMany({
      include: {
        ventes: true,
        biensAssignes: {
          include: {
...
app.get('/api/compta/tresorerie', authMiddleware, async (req, res) => {
  try {
    const rentPayments = await db.paiement.findMany({
      where: { type: 'loyer', statut: 'paye' },
      include: { contrat: { include: { bien: { include: { proprietaire: true } } } } }
...
app.get('/api/compta/creances', authMiddleware, async (req, res) => {
  try {
    const activeContracts = await db.contrat.findMany({
      where: { actif: true },
      include: {
    ...
    const paymentsLate = await db.paiement.findMany({
    ...
    const paymentsAll = await db.paiement.findMany({
      include: { contrat: { include: { locataire: true, bien: true } } },
      orderBy: { datePaiement: 'desc' },
      take: 30
    });
...
app.get('/api/ventes', authMiddleware, async (req, res) => {
  try {
    const list = await db.vente.findMany({
      include: {
        bien: { include: { proprietaire: true } },
        agent: true
      },
...
app.post('/api/ventes', authMiddleware, requireRole(['admin', 'agent', 'comptable']), async (req, res) => {
...
// GET report PDF
app.get('/api/compta/rapport-pdf', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const allTransactions = [...rentIncomes, ...saleIncomes, ...manualIncomes, ...expTransactions].sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
    );
...
app.get('/api/compta/export-csv', authMiddleware, async (req, res) => {
  try {
    ...
    const all = [...rentIncomes, ...saleIncomes, ...manualIncomes, ...expTransactions].sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
    );
...
app.get('/api/compta/quittance/:paymentId', async (req, res) => {
  ...
  try {
    const payment = await db.paiement.findUnique({
      where: { id: paymentId },
      include: {
        contrat: {
          include: {
    ...
    if (!contrat) return res.status(404).json({ error: 'Contrat associé introuvable' });
    const locataire = contrat.locataire;
    const bien = contrat.bien;
    if (!locataire || !bien) return res.status(404).json({ error: 'Locataire ou bien associé introuvable' });
    const proprietaire = bien.proprietaire;
    if (!proprietaire) return res.status(404).json({ error: 'Propriétaire introuvable' });
    ...
    return generateQuittancePDF(res, {
      enterprise: appSettings.enterprise,
      proprietaire: {
        ...proprietaire,
...
app.get('/api/personnels', authMiddleware, async (req, res) => {
  try {
    const list = await db.personnel.findMany({
...
app.post('/api/personnels', authMiddleware, requireRole(['admin']), async (req, res) => {
  ...
  try {
    const newStaff = await db.personnel.create({
      data: {
        nom,
        poste,
        email,
        ...
        permissions: permissions ? (permissions as Prisma.InputJsonValue) : Prisma.DbNull,
        absences: [],
        planning: {
          "Lundi": "09:00 - 17:00",
          "Mardi": "09:00 - 17:00",
    ...
    await db.user.updateMany({
      where: { email: email.toLowerCase() },
      data: { permissions: permissions ? (permissions as Prisma.InputJsonValue) : Prisma.DbNull }
    });
...
app.put('/api/personnels/:id', authMiddleware, requireRole(['admin']), async (req, res) => {
  const { id } = req.params;
  ...
  try {
    const updated = await db.personnel.update({
      ...
      data: {
        ...
        permissions: permissions !== undefined ? (permissions ? (permissions as Prisma.InputJsonValue) : Prisma.DbNull) : undefined
      }
    });
    ...
    if (updated.email) {
      await db.user.updateMany({
        ...
        data: { permissions: updated.permissions ? (updated.permissions as Prisma.InputJsonValue) : Prisma.DbNull }
      });
    }
...
app.delete('/api/personnels/:id', authMiddleware, requireRole(['admin']), async (req, res) => {
  const { id } = req.params;
...
// Absences Request
app.post('/api/personnels/:id/absences', authMiddleware, async (req, res) => {
  const { id } = req.params;
  const { dateDebut, dateFin, motif } = req.body;
  
  try {
    const staff = await db.personnel.findUnique({ where: { id } });
    if (!staff) return res.status(404).json({ error: 'Personnel introuvable' });

    const absencesList = (staff.absences as any[]) || [];
    const newAbs = {
...
app.put('/api/personnels/:id/absences/:absId', authMiddleware, requireRole(['admin']), async (req, res) => {
...
app.get('/api/settings/users', authMiddleware, requireRole(['admin']), async (req, res) => {
  try {
...
app.post('/api/settings/users', authMiddleware, requireRole(['admin']), async (req, res) => {
...
app.put('/api/settings/users/:id', authMiddleware, requireRole(['admin']), async (req, res) => {
  const { id } = req.params;
...
app.post('/api/locataires/:id/send-receipt', authMiddleware, requireRole(['admin', 'agent', 'comptable']), async (req, res) => {
  const { id } = req.params;
  
  try {
    const loc = await db.locataire.findUnique({ where: { id } });
